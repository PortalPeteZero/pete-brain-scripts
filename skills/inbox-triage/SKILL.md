...
17. **A BULK label operation MUST be reversible — write a manifest.** Any mass label mutation outside the per-thread ops table (stripping a project/engine label off a batch of junk, a bulk re-label, a backfill) writes one JSON line per thread — `{thread_id, labels_removed, labels_added, why, when}` — to a manifest file AND records the count + file path in the session log, before/as it runs. `triage-log.py` already takes `--manifest`; a hand-rolled bulk loop must do the same. **Why:** on 23 Jul 2026 a 35-thread strip of junk off `Projects/SY-Training-Enquiries` was correct, but no manifest was written, so the exact set could not be independently re-enumerated or reversed afterwards — only reconstructed by inference. A bulk mutation you cannot list afterwards is a bulk mutation you cannot undo.
