...
When matched, **don't classify it as a generic `reply`/`task`** — and **do NOT draft a reply, do NOT run `te-log`, do NOT touch the CRM**. Triage's ONLY job for an enquiry is to **RECOGNISE and ROUTE** it. The Enquiry Engine is a **separate process** (`enquiries` sweep / `reply to enquiry in {company}`, Pete-triggered) that owns ALL drafting, precedent retrieval, `te-log` capture, CRM stage moves, and sending — with the reply properly threaded to the incoming email. **Triage drafting enquiry replies is a hard error (Pete, 30 Jun 2026: "what the fuck are you playing at" — orphan drafts not linked to the incoming thread, and enquiry work done in triage instead of the EE which runs separately).**

Routing an enquiry in triage = exactly these three side-effects, nothing more:
- **Apply the `Projects/SY-Training-Enquiries` filing label AND the `Replies` label** (both). The TE label is the Engine's home; the `Replies` label is Pete's at-a-glance "this enquiry is in-flight / needs handling" view (Pete, 30 Jun: "otherwise we dont know they have been dealt with"). The `Replies` label is stripped later by the EE when the reply is actually sent.
- **Raise a chase task** in `public.tasks` (`project_slug='SY-Training-Enquiries'`) capturing the ask **in the customer's own words** — never assert a course/term mapping you haven't verified (see [[2026-06-30-dont-assert-mappings-from-filename-matches]]).
- In the ops table mark `Ask = reply`, Action **"Route → Enquiry Engine"**, note both labels. **No draft. No send.** (A booking forward — "Sent to Sue" — moves the contact to **Customer/won** in the EE, not here.)
...
- **MANDATORY task reconciliation — every item, no exceptions (Pete standing rule, 2026-06-28).** As part of dealing with EVERY tray item, search `public.tasks` for related open tasks — match on sender / company / subject / topic (e.g. `name ILIKE '%{keyword}%'`), not just an explicitly-linked task id. Then in the SAME breath as handling the item: **close** any task the email shows is already done, **surface** any task that still needs action, and **propose** a new task if the email implies work and none exists. Reconcile the task state before/at de-tray — never make Pete ask "are there any open tasks?" or "update the tasks". An item is not "dealt with" until its task state is reconciled.
