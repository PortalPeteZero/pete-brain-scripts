...
|---|---|---|---|
| `brain/` | v6.6 | Cowork + Claude Code | Command Centre session layer (sessions, reviews, tasks, memory, meetings, output styles). Multi-system context loading. EE section now leads with the **⛔ DRAFT GATE** (`ee-draft-gate.py` before any reply is shown — never hand-draft), plus **look it up before asking Pete** and **corrections become banked enforced rules** (measured against real approved replies before going fail-closed). Draft gate now enforces full-thread read + CRM-first mechanically. Full history: [[CHANGELOG]]. |
...
| `inbox-triage/` | v1.13 | Cowork + Claude Code | Interactive inbox walker, front-end of the **Triage Engine** (facts-first read-and-judge, every decision captured to `triage_decisions` via `triage-log.py`, overrides bank regression cases; boot pack: [[triage-manifest]]; design: [[triage-engine-design]]). Presents in ACTION-TYPE groups one at a time (no-action pile → team → reply → task). Verb `triage`. `Hand to {person}` creates a CC task (`public.tasks`) under `project_slug='General'`, tagged `delegated`. Full history: [[CHANGELOG]]. |
