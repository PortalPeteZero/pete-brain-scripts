...
| `inbox-triage/` | v1.11 (2026-05-06) | Cowork + Claude Code | Interactive inbox walker. Verb `triage`. **2026-05-06 (v1.11)**: Delegated workflow updated -- `Delegate to {person}` now creates the Asana task in **Team-General** (gid `1214564987703466`) placed in the **Delegated** section (gid `1214564987864352`). Old standalone Delegated project archived. Routing chain follows asana-gmail-sync v1.7. **(v1.10)** TitleCase enrichment paths. |
| `asana-gmail-sync/` | v1.7 (2026-05-06) | Cowork + Claude Code | Reconciliation engine. Verb `sync asana`. **2026-05-06 (v1.7)**: routing chain rewritten for project consolidation. Step 5 Delegated now points at Team-General/Delegated section. Step 6 orphan handling: Customers/Suppliers → Team-General/`{prefix}-General` section (SY-Clancy exception → standalone SY-Clancy project). Invoices/* → Team-Finances/`{prefix}-Invoices` section. Accreditations/* → Team-General/SY-General. Articles labels → website parents' `articles` sections. Default to General sub-project; ask before sprawling new sub-projects. **(v1.6)** PA- consolidation. |
