...
| `vault-writer/` | v5.7 | Cowork + Claude Code | End-of-session cleanup checklist (structured-home sweep, task staleness, task↔project parity). Full history: [[CHANGELOG]]. |
| `closeout/` | v1.0.2 | Cowork + Claude Code | End-of-session command for any property-touching session (SEO site, LeakGuard, the CC, or a new build). Proves this session's own commits from the transcript's `gitOperation` stamp (`session_attribution.py`), records only its own work (Work Log via `closeout-sweep.py`, knowledge to its correct CC home, the diary), verifies live state, and hands Pete one menu of judgement calls. Safe beside parallel sessions. Trigger: `/closeout`, "close out", "wrap this up", "are we done?". Full history: [[CHANGELOG]]. |
