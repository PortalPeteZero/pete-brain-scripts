...
|---|---|---|---|
| `brain/` | v6.4 | Cowork + Claude Code | Command Centre session layer (sessions, reviews, tasks, memory, meetings, output styles). Multi-system context loading. EE capture-on-send now requires `draft_text` + enforced source-fix on edits; adds the `EE sign off` verb. Full history: [[CHANGELOG]]. |
