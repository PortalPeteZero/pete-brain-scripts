...
|---|---|---|---|
| `brain/` | v6.7 | Cowork + Claude Code | Command Centre session layer (sessions, reviews, tasks, memory, output styles). Meetings moved out to `meeting-notes/`. Multi-system context loading. EE section now leads with the **⛔ DRAFT GATE** (`ee-draft-gate.py` before any reply is shown — never hand-draft), plus **look it up before asking Pete** and **corrections become banked enforced rules** (measured against real approved replies before going fail-closed). Draft gate now enforces full-thread read + CRM-first mechanically. Full history: [[CHANGELOG]]. |
| `vault-writer/` | v5.9 | Cowork + Claude Code | End-of-session cleanup checklist (structured-home sweep, task staleness, task↔project parity, EE capture + sign-off mirror of closeout I3). Full history: [[CHANGELOG]]. |
| `closeout/` | v1.0.4 | Claude Code (auto-record needs the CC transcript; verify/surface checks work anywhere) | End-of-session command for any property-touching session (SEO site, LeakGuard, the CC, or a new build). Proves this session's own commits from the transcript's `gitOperation` stamp (`session_attribution.py`), records only its own work (Work Log via `closeout-sweep.py`, knowledge to its correct CC home, the diary), verifies live state, and hands Pete one menu of judgement calls. Check I3 now also verifies the EE `draft_text` was captured (I3a) + the EE sign-off is clean — no unfixed source-bearing edit (I3b). The commit auto-record needs the Claude Code interactive transcript; where it's absent (e.g. Cowork) the gate refuses loudly (exit 3) rather than giving a false all-clear. Safe beside parallel sessions. Trigger: `/closeout`, "close out", "wrap this up", "are we done?". Full history: [[CHANGELOG]]. |
...
| `inbox-triage/` | v1.14 | Cowork + Claude Code | Interactive inbox walker, front-end of the **Triage Engine** (facts-first read-and-judge, every decision captured to `triage_decisions` via `triage-log.py`, overrides bank regression cases; boot pack: [[triage-manifest]]; design: [[triage-engine-design]]). Presents in ACTION-TYPE groups one at a time (no-action pile → team → reply → task). Verb `triage`. `Hand to {person}` creates a CC task (`public.tasks`) under `project_slug='General'`, tagged `delegated`. Full history: [[CHANGELOG]]. |
...
| `meeting-notes/` | v1.0 | Cowork + Claude Code | Recorded meeting → detailed written record → **discussion** of next actions. Plaud transcribes; Claude writes everything else (Plaud's AI summary is navigation only — it invents due dates and misattributes actions, measured 30 Jul 2026). Detail is the brief: ~1,500–3,000 words for 45 min. Nothing created until Pete decides — tasks, projects, delegation or nothing. In-session only, never scheduled. Full history: [[CHANGELOG]]. |
...
| `sygma-health-report/` | v1.2 | Cowork + Claude Code | Combined multi-source website health report for sygma-solutions.com — Ahrefs + GSC + GA4 + Google Ads (Surfer dropped 2026-06-08). Deep-dives the 4 same-course cluster pages (EUSR CAT1, Cat & Genny, Cable Avoidance, HSG47) + GSC daily cross-check table (judge on GSC, never Ahrefs alone). Triggers: "run the sygma report", "how's the sygma website". Output: auto-published snapshot to CC `reports.snapshots` (`sygma-health`) → /m/sygma-reports Health tab. Read-only (website carve-out); honours hsg47-explained restricted rule (no ranking pitches; tech fixes fine) + no-backlinks. Script: `skills/sygma-health-report/scripts/build_report.py`. |
...
| `skill-builder/` | v1.0 | Cowork + Claude Code | How to build, edit, package, and retire custom skills. Trigger: "build a skill", "create a skill", "new skill", "package a skill". Full history: [[CHANGELOG]]. |
