...
| `inbox-triage/` | v1.12 | Cowork + Claude Code | Interactive inbox walker, now the front-end of the **Triage Engine** (facts-first classification, every decision captured to `triage_decisions` via `triage-log.py`, overrides bank regression cases; boot pack: [[triage-manifest]]; design: [[triage-engine-design]]). Verb `triage`. `Hand to {person}` creates a CC task (`public.tasks`) under `project_slug='Team-General'`, tagged `delegated`. Full history: [[CHANGELOG]]. |
