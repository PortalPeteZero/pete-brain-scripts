...
| `closeout/` | v1.0.3 | Claude Code (auto-record needs the CC transcript; verify/surface checks work anywhere) | End-of-session command for any property-touching session (SEO site, LeakGuard, the CC, or a new build). Proves this session's own commits from the transcript's `gitOperation` stamp (`session_attribution.py`), records only its own work (Work Log via `closeout-sweep.py`, knowledge to its correct CC home, the diary), verifies live state, and hands Pete one menu of judgement calls. The commit auto-record needs the Claude Code interactive transcript; where it's absent (e.g. Cowork) the gate refuses loudly (exit 3) rather than giving a false all-clear. Safe beside parallel sessions. Trigger: `/closeout`, "close out", "wrap this up", "are we done?". Full history: [[CHANGELOG]]. |
