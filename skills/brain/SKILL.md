...
**⛔ DRAFT GATE FIRST — never hand-draft an enquiry reply into the chat.** Every `reply`/`quote` MUST be produced through **`ee-draft-gate.py --in <payload>`** BEFORE it is shown to Pete. It re-runs the Sent-history reconciliation live, re-resolves the course via `ee-facts`, demands the three must-haves with the customer's own words as evidence, derives the stage from the balance rule, runs the FULL `ee-lint` shift-left, checks the banked rules the draft must honour, and stamps the draft; `ee-send` REFUSES an unstamped or since-altered draft. **It also PULLS the live thread and blocks unless EVERY inbound customer message is represented in `incoming_text`** — skimming the latest message is now mechanically impossible — **and reads the contact's CRM history (by address OR company domain) at DRAFT time, blocking until you declare `classification.crm_reviewed = {"activity_count": N}`**. So "read the whole thread" and "check the CRM first" are enforced, not merely instructed. **If you catch yourself composing a reply in the chat window, STOP and run the gate.** This is THE recurring failure: it caused the 21 Jul 2026 Wheal Jane freelance-draft miss the gate was built for, and it happened again on 22 Jul 2026 — five corrections on a single reply, purely because the gate was skipped.
...
**⛔ Look it up before you ask Pete.** The engine already holds the facts: per-customer rates in `ee_customer_rates` (**company-scoped** — e.g. Clancy £850/day resolves for every Clancy contact, not the standard £965), standard prices in `ee_rates`, course cap/cert/agenda attachments in `ee_catalogue` + `ee-facts.py`, open-course dates in `public.ee_public_courses`, and a customer's commercial terms in that customer's own records. Asking Pete for something the system already knows is a failure (22 Jul 2026: asked for Clancy's day rate when it was in the partnership agreement at Clause 5.1, and asked which Super User agendas exist when all three were live and linked).

**⛔ A correction becomes a BANKED, ENFORCED rule in the SAME session.** A rule that only lives in the chat is invisible to the next session and WILL recur (the fill-the-day upsell recurred for weeks because nobody banked it). Bank it in `ee_rules` with `scenarios` + `applies_when` + `require_pattern` + `fail_hint`: a rule carrying a `require_pattern` is **hard-enforced fail-closed** by `ee-draft-gate`; without one it is advisory (surfaced to every relevant draft, never blocks). **Never enable a fail-closed rule without first measuring it against real approved replies** — a rule that blocks Pete's own sent quotes is far worse than no rule (22 Jul 2026: a new enforcement set would have blocked **22 of 25** approved quotes until it was measured and loosened to 3).

**Precedents = phrasing, not facts (§4A / §8.5).** Resolve the course + facts deterministically with `ee-facts.py` (cap/cert/agenda from the facts index); take **prices + cert options + attachments** from the EE's CC tables (`ee_rates` + `ee_customer_rates` + `ee_catalogue`, all resolved by `ee-facts.py`; lessons land there via `ee-learn.py`, never notes), **dates** from the live source (`public.ee_public_courses`). A retrieved reply supplies tone/structure only — its figures are time-stamped context, NEVER current fact (seeded historical examples are marked `historical`/`figures_illustrative`).

**⛔ Capture-on-send is a hard gate.** A send is NOT done until `te-log --apply` has run for it (CRM activity + stage, knowledge note, chase task) — this holds for EVERY exit path, including just sending a queued draft or a batch of signed-off drafts, not only the `enquiry` loop. **Never de-tray an enquiry by hand** — `te-log --apply` does de-tray + archive + CRM + knowledge + chase in one; manual label-stripping silently skips the capture (failure 30 Jun 2026). After capture, close any duplicate chase the triage routing already made. **Pass `draft_text` (your held draft) on every `--apply`** so the edit-free/edited signal is captured; **when Pete edited, you MUST also trace the correction to the source that misled the draft and FIX that source** (`source_ref` + `source_fix`; the `correction_category` is the pointer) — the fix is a **DB edit via `ee-learn.py`** (which sets `source_fixed` back on the touch and logs `ee_edits`), **never a note**; a source-bearing edit is not captured until named and not reconciled until fixed (enforced by the DB CHECKs + the EE sign-off). Full detail: [[workflow-design]] §6.5a / §6.10.

**⛔ EE sign-off reconciles before done.** No enquiry session is done until `ee-signoff.py --since today` prints zero on every line — every reply captured, EVERY source-bearing correction's source fixed (`source_fixed=true`), tray clear, chases set. It exits non-zero while anything is outstanding. Wired into `closeout` check I3.

Verbs: **`enquiry`** (handle one inbound) · **`enquiries`** (manual sweep, Pete-triggered, NO cron) · **`reply to enquiry in {X}`** (the loop on one) · **`EE sign off`** / **`reconcile enquiries`** (the mandatory end-of-session reconciliation — runs `ee-signoff.py`; also the closing step of the `enquiries` sweep; done = zero unfixed sources) · **`Sent to Sue`** (booking handoff → Customer/won). All follow [[workflow-design]].
...
6b. **Claude Exchange check (the Jim channel -- standing, added 4 Aug 2026)** -- Pete's Claude and Jim's Claude share a Google Chat space, **Claude Exchange** (`spaces/AAQA66Jdho0`); long payloads travel as FILES in Sygma Hub `_System/claude-exchange/` and the space carries the pointer (a file drop with no space message = NOT delivered -- the request-07 lesson: a request sat in the folder and never reached Jim's session). At resume, read it:
   `VAULT=/tmp/pbs python3 /tmp/pbs/chat-api.py messages spaces/AAQA66Jdho0 15`
   - Surface anything tagged `[Jim's Claude]` newer than the last session that is unanswered -- an uploaded reply file, a question, a completion line. Briefing line: `**Claude Exchange**: {N} new from Jim's Claude -- {one-liner each} -- say "check exchange" to walk them.` Skip the line when nothing new.
   - If a message announces a reply FILE, fetch it from the Hub folder and read it BEFORE acting on anything that depended on it.
   - **Surface only -- never reply automatically at resume.** Mid-session sends follow the space protocol (its pinned first message): every message opens with `[Pete's Claude]`, anything expecting an answer ends "Reply in this space.", every finished job gets a one-line completion (+ reply filename). No secrets, no personal-note contents -- filenames and pointers only.
...
8. **Present briefing** -- Concise standup format:
   ...
   **Claude Exchange**: [N new from Jim's Claude -- one-liners; skip if none]
...
7d. **Claude Exchange sign-off (outgoing -- standing, added 4 Aug 2026)** -- if this session touched a SHARED property (Locator Data, Sygma Platform, or anything Jim also works), or acted on / created a `_System/claude-exchange/` request, close the loop on the space (`spaces/AAQA66Jdho0`):
   - Post the one-line completion tagged `[Pete's Claude]` -- what shipped / what was uploaded, with the reply filename if there is one. A file left in the folder without a space message counts as NOT delivered.
   - Read the space for unanswered `[Jim's Claude]` messages and SURFACE them to Pete before closing -- never leave his Claude waiting silently over a session boundary.
   - Protocol lives in the space's pinned first message + Sygma Hub `_System/README.md`. No secrets, pointers only.
