...
description: >
  Reconciliation engine for Pete's email-workflow system. (Name is historical
  — Pete is off Asana since 24 Jun; this skill now syncs Gmail ↔ the CC
  `public.tasks` table, not Asana. Asana is Jane's only.) Bidirectional sync
  between Gmail labels and CC task state. Auto-creates CC tasks for
  Actions-labelled orphans (no asking) using smart routing, defaulting priority
  to P2 when no signal. Closes CC tasks (status='done') when Actions or
  Delegated label is removed manually in Gmail. Strips Gmail Actions /
  Delegated labels when CC tasks are done. Detects demand-driven label
  opportunities, auto-filter patterns, and parity drift between Gmail labels and
  vault folders. Surfaces all suggestions for Pete's confirmation. Closure
  exemptions per the 2026-06-06 Action/Task split: [no-sync-close] marker +
  Team-Finances blanket; every sync closure gets an audit comment. Runs on
  command -- idempotent, safe to run repeatedly. Triggered by the phrase "sync
  asana" or any reconciliation request. Offered (opt-in) at the end of every
  triage session, and runs daily at 07:15 as the daily-asana-gmail-sync cron.
---
...
Reconciliation engine for the email workflow. Bidirectional sync between Gmail labels (`Actions`, `Delegated`) and **CC `public.tasks`** state (Pete off Asana 24 Jun — Asana is Jane's only). The verb `sync asana` runs this skill (verb name kept for muscle memory; the target is the CC task table, not Asana).
...
- CC task store: `public.tasks`, CRUD via `VAULT=/tmp/pbs python3 /tmp/pbs/cc-sql.py` (`name`, `priority`, `due_on`, `entity_slug`, `project_slug`, `notes`, `status`). Pete off Asana 24 Jun — Asana is Jane's only; do NOT connect to it for Pete's work.
...
2. **Bidirectional**. Mark a CC task `status='done'` → label leaves Gmail. Close in Gmail (remove Actions/Delegated label) → CC task set `status='done'`.
3. **Auto-create, never auto-structure**. CC tasks (`public.tasks` rows) for Actions/* orphans are created without asking. New labels, folders, filters always require Pete's confirmation via the proposal pattern.
4. **The CC `public.tasks` table is source of truth for task STATE** (priority, completion). Gmail labels are a view -- they follow CC task state.
5. **Re-prioritisation lives in the CC only**. There's no Gmail sub-label to swap. Pete edits the `priority` column on the `public.tasks` row directly when he wants to re-prioritise. The "close-on-label-removed" rule fires only when NEITHER `Actions` NOR `Delegated` is on the thread. **When priority changes, the due date does NOT auto-recalculate** -- if Pete changes a task from P3 to P1, the existing `due_on` stays unless Pete edits it. Re-pri shouldn't accidentally pull a deadline 28 days closer.
...
> [!important] Target is `public.tasks`, not Asana (Pete off Asana 24 Jun). The wrapper's task-state operations (Steps 1/3/4) and the orphan candidates it surfaces (Step 6) all act on the CC `public.tasks` table — every "create/close/read a task" below means a `cc-sql.py` INSERT/UPDATE/SELECT against `public.tasks`, NOT an Asana call. Asana is Jane's only.

**After running the wrapper:**
1. Read its output (closures, label strips, parity, orphan candidates).
2. For each Step 6 orphan candidate, decide routing per the fallback chain below and call the CC task-creation path (`INSERT INTO tasks (…, project_slug, …)` into `public.tasks` — see "Orphan task creation" section). Apply the locked Mimestream + Gmail + Finder link policy, and call `vault-enricher.py` on the source thread → target vault folder.
...
Query the CC `public.tasks` table **twice** — once for open tasks, once for tasks marked done in the last 30 days — and merge into the linked set. Both have linked Gmail threads that the algorithm needs to act on. (Pete off Asana 24 Jun — this is a `cc-sql.py` SELECT against `public.tasks`, NOT an Asana query.)
...
For each task, extract Gmail thread IDs from notes (matching both `mail.google.com/mail/u/0/#[a-z]+/{thread_id}` and `links.mimestream.com/g/{email}/t/{thread_id}` URL forms). Carry forward: `id`, `name`, `status`, `due_on`, `entity_slug`, `project_slug`, the thread IDs, and the `priority` value.

**Both-sides query is non-negotiable.** If Step 1 only pulls open tasks, Step 3 has no done tasks to act on — the Gmail label persists after CC-side closure, and Step 6 then auto-creates a duplicate ("revives" the task). The wrapper enforces both queries on every run. See [[Library/lessons/2026-05-20-sync-must-query-both-open-and-closed-tasks]].

### Step 2: Priority reconciliation — no-op

No Gmail sub-labels for priority; the CC `public.tasks` `priority` column is the sole authority. Step 2 reports nothing. Step number preserved for ordinal stability.

### Step 3: Completion reconciliation (CC → Gmail)

For each CC task whose `status = 'done'` in the linked set, strip the `Actions` label or the `Delegated` label from the Gmail thread (whichever is present). Use `gmail-api.py modify_thread` with `remove=[label_id]`.
...
For each linked task that is still open (`status != 'done'`), check the linked Gmail thread's current labels:

- If thread has the `Actions` label OR the `Delegated` label → leave the task alone (it's still active).
- If thread has NEITHER `Actions` NOR `Delegated` → **set the CC task `status='done'`** (`UPDATE tasks SET status='done' WHERE id=...` via `cc-sql.py`). Pete handled this action in Gmail (or via another route) and removed the workflow label. Sync follows.

**Exemptions (Action/Task split, locked 2026-06-06 — the wrapper enforces both):**

1. **`[no-sync-close]` marker in task notes** → NEVER close on label state. Two uses: (a) Pete-sent watch tasks (chase-if-no-reply; thread never had Actions — added 2026-05-24); (b) **CC-only tasks** — bills, cert batches, work items created by the `Task this` verb or de-trayed by Pete. Their work happens outside email, so Gmail label state must never close them.
2. **Team-Finances blanket** — any task with `project_slug='Team-Finances'` is exempt regardless of marker. A bill is never a reply.

**Closure audit note** — every task Step 4 closes gets an audit line appended to the CC task `notes`: *"Closed by sync — Actions/Delegated label removed in Gmail, {date}. If this strip was a tray clear-out rather than completion, reopen (`status='open'`) and ask Claude to mark it [no-sync-close]."* Closures are also listed in the running session's / cron's daily-note block. This is the safety net for the strip-to-clear-out gesture.

**The one-sentence rule behind the split: Actions = waiting on Pete to respond by email. Everything else = CC task only.** Design + migration record: [[Projects/PA-General/files/email-workflow-plan-2026-06-06-action-task-split]] and [[Library/decisions/2026-06-06-actions-label-reply-only]].
...
- Thread was deleted (DELETE /threads) → no thread to check. If the Gmail thread URL returns 404, close the task (`status='done'`) with an appended note: "Source thread deleted from Gmail."
...
For each open CC task on the Delegated track — `project_slug='Team-General'` with a `[delegated]` marker (or `Delegated` in notes) recorded at delegation time:
...
5. If a genuine reply found → set the CC task `status='done'` + remove `Delegated` Gmail label + append the same audit-note pattern as Step 4 ("Closed by sync — {delegatee} replied {date}"). Report: "✓ Jane replied to the Clancy Q2 delegation -- closed the task." (Here Jane is the delegatee — a person Pete delegated to — not Jane's Asana.)
...
Find Gmail threads labelled `Actions` or `Delegated` that have NO matching CC task in `public.tasks` (no row whose `notes` link to the thread id).
...
For each orphan, **auto-create the task with smart routing** (no asking). Pete is off Asana 24 Jun — **the task is a row in `public.tasks`, created with `cc-sql.py`, NOT an Asana task. There are no GIDs, no sections, and no custom-field priority: smart-routing resolves to an `entity_slug` + a `project_slug` NAME, and priority is the `priority` column.**
...
- **Routing discipline**: the fallback chain below implements the task-routing decision tree at `[[vault-routing#task-routing-decision-tree]]` — related project first, else `{prefix}-General`; project escalation only ever by proposal to Pete, never auto-created.
- **Priority**: no priority signal in Gmail (single-Actions-label model). Default to **P2** (sensible middle priority) in the `priority` column. Pete can edit the row. For `Delegated`-only orphans, leave `priority` unset (Delegated is its own track).
- **Smart-routing fallback chain (Gmail label → `entity_slug` + `project_slug`)**:
  1. Thread has a `Projects/*` label → `project_slug` = the project NAME from the label (e.g. `'CD-Website'`); `entity_slug` = the same project slug. (For demand-driven sub-project labels, route to the parent project NAME.)
  2. Thread has a `Customers/*` or `Suppliers/*` label only → `entity_slug` = the customer/supplier slug from the label; `project_slug` = the matching `{prefix}-General` NAME:
     - SY → `project_slug='Team-General'` (the `SY-General` work lives here)
     - CD → `project_slug='Team-General'`
     - EA → `project_slug='Team-General'`
     - AT → `project_slug='AT-General'` (Ashcroft Family) -- AT- supplier vault home is `Personal/family/`
     - **SY-Clancy exception**: threads labelled `Customers/SY-Clancy` route to `project_slug='SY-Clancy'` (its own project NAME), with `entity_slug='SY-Clancy'`, NOT Team-General.
  3. Thread labelled `Invoices/*` (legacy label, kept for backwards compatibility) → `project_slug='Team-Finances'`; `entity_slug` = the invoice owner slug:
     - `Invoices/CD-Invoices` → `project_slug='Team-Finances'`, `entity_slug='CD-Invoices'`. (Section status — To Pay / Overdue / Awaiting — is no longer a GID; record it in `notes` if known. Default for new payables: "To Pay". Auto-paid items still route to `Receipts`, not Invoices — see CLAUDE.md rule.)
     - `Invoices/SY-Invoices` → `project_slug='Team-Finances'`, `entity_slug='SY-Invoices'`. Default state "To Pay".
     - `Invoices/EA-Payments` → `project_slug='Team-Finances'`, `entity_slug='EA-Payments'`.
  4. Thread labelled `Accreditations/*` → `project_slug='Team-General'`, `entity_slug` = the accreditation slug (e.g. `'SY-ProQual'`, `'SY-EUSR'`); vault folders at `Accreditations/SY-ProQual/` and `Accreditations/SY-EUSR/` remain reference-only.
  5. **Personal/PA-{area} label** → `project_slug='PA-General'`; `entity_slug` = the area slug (record the area in `notes` so it stays grouped):
     - `Personal/PA-Scouts` → `entity_slug='PA-Scouts'`
     - `Personal/PA-Los-Claveles` → `entity_slug='PA-Los-Claveles'`
     - `Personal/PA-Freemasonry` → `entity_slug='PA-Freemasonry'`
     - `Personal/PA-Finance` → `entity_slug='PA-Finance'`
     - `Personal/PA-PassionFit` → `entity_slug='PA-PassionFit'`
  ...
  7. Thread has no filing label and no folder match → default to `project_slug='PA-General'` (no `entity_slug`). Pete cleans up periodically -- the trade for zero friction.
...
- **Task notes**: include all three of these (Mimestream first):
  ...
  3. **Finder link to the matching vault folder** (when the task is tied to a project/customer/supplier): generated via `/tmp/pbs/vault-finder-link.py {project-name} [section-name]`. Returns a `file:///Users/peterashcroft/Second%20Brain/...` URL that opens the folder in macOS Finder. Omit if no matching vault folder exists (don't emit a broken link).

  Then: brief summary + how it was routed (which fallback step matched) + "priority defaulted to P2 — edit the `priority` column if needed".
...
- **Due date** (`due_on`) -- derived from the **default priority P2** (Atlantic/Canary tz): today + 7 days. Auto-due-date schedule still applies for any priority Pete later sets:
  - P1 → today + 2 days (48h)
  - P2 → today + 7 days (1 week)
  - P3 → today + 30 days (1 month)
  - P4 → no due date
  - `Delegated`-only orphans → no due date here (Step 5 handles delegation timing)
  - Pete can edit `due_on` directly after creation. The default schedule is a starting point, not a constraint.
- **Owner**: always Pete — these are Pete's CC tasks. (No assignee GID; the table is Pete's.)
...
- **Project label ↔ vault folder ↔ CC project**: every demand-driven `Projects/{slug}` Gmail label has matching vault folder + a `project_slug` in use on `public.tasks`. Surface drift. (Asana projects are Jane's — not in scope here.)
...
DELEGATED:
- ✓ Jane replied to {thread} -- closed (2 days ago)   [Jane = delegatee, not Jane's Asana]
- ⚠ 3 overdue chasers drafted to Drafts:
    - {subject} to {delegatee} -- 9 days overdue
    - ...

ORPHANS auto-created (no asking, default P2):
- {subject} → project_slug=Team-General, entity_slug=SY-Smith, P2 (sender matched Customers/SY-Smith → fell to {prefix}-General)
- {subject} → project_slug=SY-Clancy, entity_slug=SY-Clancy, P2 (Customers/SY-Clancy exception — own project NAME, not Team-General)
- {subject} → project_slug=Team-Finances, entity_slug=CD-Invoices, P2 (Invoices/CD-Invoices label)
- {subject} → project_slug=PA-General, entity_slug=PA-Scouts, P2 (Personal/PA-Scouts label)
- {subject} → project_slug=PA-General, P2 (no filing label, no folder match -- default fallback)
...
- "Insurance docs from AXA" routed to project_slug=Team-General (entity_slug=CD-General) × 4 -- awaiting confirmation. Promote to Master routing matrix? (y / n)
...
- **Orphan auto-create proceeds** exactly as in interactive mode (it never asks anyway) — but **best-match routing only**: never create labels or new `project_slug` values. Ambiguous orphans → `project_slug='PA-General'` + flagged for re-route.
...
- **Failure escalation**: 2 consecutive failed runs → P2 CC task (`project_slug='Team-General'`, `entity_slug='SY-General'`) in `public.tasks` (mirrors staff-master-sync).
...
6. **The CC `public.tasks` table is source of truth for priority and completion**. Gmail labels are a view on top. (Asana is Jane's only.)
...
8. **Auto-create tasks (Actions/* orphans only) -- no asking**. The smart routing fallback is fast and predictable. `project_slug='PA-General'` is the explicit dump for unmatched orphans -- accepted trade for zero-friction triage.
...
11. **Re-prioritisation is not closure**. The "close on label-removed" rule fires only if NEITHER `Actions` NOR `Delegated` remains on the thread. Priority changes happen on the `public.tasks` `priority` column directly (no Gmail sub-label to swap).
...
Claude:
1. Uses `cc-sql.py` for all task CRUD against `public.tasks` (Pete off Asana 24 Jun — no Asana tools loaded for Pete's work; Asana is Jane's only).
