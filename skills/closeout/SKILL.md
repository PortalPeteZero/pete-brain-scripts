...
| A12 | **the session deployed or edited an EDGE FUNCTION: is what is RUNNING what is in the repo?** `git log` on a function folder answers "when did the source change", never "what is live" -- on 6 Aug 2026 a hardening sweep committed 17 June was found still undeployed, eight functions serving their 1 May build for seven weeks, three of them on the Xero path. Gate: `VAULT=/tmp/pbs python3 /tmp/pbs/edge-parity.py --repo <checkout> [--ref <project>]` (run `--self-test` first; it refuses to report a number until it passes known-good cases). Exit 0 = every deployed function matches source, 1 = something behind, 2 = could not check. **`NO-SOURCE` is its own status** -- a function deployed with no source in the repo cannot hide inside a staleness count, which is how two were found running on locator-data with nothing to review them against | verify | |
...
| I6 (Property sign-off — **BLOCKING** for properties touched this session) | (C2) the session shipped to a website/app repo → assert that repo/domain has a `property_declarations` row (the card mandate exists in property-manager §0d but is prose-only + intake-path-only; a repo shipped without that intake is otherwise never caught). (C3) the session onboarded/first-filed a new customer/supplier → assert a `vault_notes` type=customer|supplier record exists (per `[[vault-routing#onboarding-rituals]]`). **Gate (19 Jul 2026): `VAULT=/tmp/pbs python3 /tmp/pbs/property-signoff.py --touched "<property-key or Name>" [...]` (prefer the immutable `key` — it survives renames; the name is accepted as a fallback) — exits non-zero while a touched property cannot say who serves it (P1), where its code lives (P2), or has a front door that does NOT resolve (P3). P4 (no front door recorded) is a WARNING only.** This closes the one category that had no gate: enquiries, triage and customer knowledge each block, properties did not, so correctness depended on Pete remembering to ask. (C3) the customer/supplier record check stays ADVISORY — surface it, never block. The daily `cc-locator-audit.py` is the report-only backstop | verify → "declare the property card / create the customer record?" |
| I7 (CC Locator drift — advisory) | this session ADDED anything the locator covers — **a skill, helper, project, table, storage bucket, property (website/app), entity or connector** — re-run it so the session's own additions are caught before you close, rather than waiting for tomorrow's 06:30 cron. Gate: `VAULT=/tmp/pbs python3 /tmp/pbs/cc-locator-audit.py --json` → read `gaps` (an INT) and `gap_types`. As of 19 Jul it also reports **`property-front-door-broken`** (HIGH — a property names a read-first note that no longer resolves) and **`drive-home-gone`** (HIGH — a `data_map` Drive home was deleted or trashed). **`gaps: 0` means nothing unfiled WITHIN ITS SCOPE — it does NOT cover CC pages, Railway crons, or the other databases (hub / CD-Leak / Odoo), and a customer or supplier is covered only insofar as it is an `entities` row. Read the `info[]` scope line before treating 0 as all-clear.** An `aborted: true` result means the check did not run at all — never read that as clean. Any `couldnt-check` finding means the check did NOT run — never read that as clean. Per Pete's standing decision (18 Jul 2026): **file the obvious ones and TELL him; ASK when the home is genuinely his call; ALWAYS ask before deleting anything.** Advisory — surface, never block. Note `--json` deliberately does NOT write a daily_log row, so running it here is free | verify → "file the {N} unfiled thing(s)?" |
| I8 (stale skill package — advisory) | this session EDITED any `skills/*/SKILL.md`: the packaged `.skill` archive must have been REBUILT after that edit, or installing it silently reverts the edit. Gate: `VAULT=/tmp/pbs python3 /tmp/pbs/package-skill.py --changed` — it compares each archive against its source. **This exact fault nearly lost two safety rules on 18 Jul: the archives were rebuilt but never committed, and `/tmp/pbs` is re-cloned every session.** So also confirm `git status skills/*.skill` is clean. **`--changed` compares the archive against its SOURCE *and* (since 19 Jul) against the DELIVERED copy in `~/Downloads/cc-skills-to-install`; where it cannot READ that folder (macOS denies some processes) it now warns on stderr rather than reporting a false clean — do not treat a bare "nothing to package" as proof the delivered copies are current if that warning is present.** | verify → "re-package and commit the changed skills?" |
| I9 (Claude Exchange — advisory) | the session touched a SHARED property (Locator Data, Sygma Platform, or anything Jim also works) OR acted on / created a request in Sygma Hub `_System/claude-exchange/`: the loop is closed on the **Claude Exchange** Google Chat space (`spaces/AAQA66Jdho0` — the Claude-to-Claude channel, standing since 4 Aug 2026). (a) a one-line completion posted, tagged `[Pete's Claude]`, with the reply filename if one — **a file left in the folder without a space message counts as NOT delivered** (the request-07 lesson); (b) the space read for unanswered `[Jim's Claude]` messages, each surfaced to Pete — never leave the other side waiting silently over a session boundary. Read: `VAULT=/tmp/pbs python3 /tmp/pbs/chat-api.py messages spaces/AAQA66Jdho0 15`. Protocol = the space's pinned first message + Hub `_System/README.md`. No secrets, pointers only | surface |
