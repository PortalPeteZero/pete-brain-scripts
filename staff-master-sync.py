    ...
    try:
        cc_rest("POST", "secrets?on_conflict=name",
                [{"name": "sygma-trainer-roster.yaml", "value": text,
                  ...
                  # "infra", not "sygma": the secrets taxonomy is locked to
                  # [binary-cert, infra, key-json, oauth-tokens, password, token] and
                  # connection-parity fails on anything else. This wrote "sygma" on EVERY run,
                  # so fixing the row alone would have been undone tomorrow. (20 Jul 2026)
                  "category": "infra"}],
