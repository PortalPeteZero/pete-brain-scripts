...
MAP = [
    ...
    ("Health / Garmin", "Personal", "CC: public.garmin_daily (metrics only)", "garmin-daily-cc.py (Railway cron — ONE JOB: pull Garmin)", "Sleep/HRV/readiness/activities. Cron is Garmin-only (no Drive, no journal). Manual refresh: cc-cron.py deploy garmin-daily-pull --run. Garmin lib = garmin-pull-lib.py (pure, cloud-native); sign-off = garmin-signoff.py. Old garmin-daily-pull.py deleted 27 Jun 2026."),
