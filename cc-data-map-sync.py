...
MAP = [
    ...
    ("Staff (Sygma)", "Sygma Platform", "Sygma Platform (rsczwfstwkthaybxhszy): hub.staff_directory = WHO WORKS AT SYGMA, the SSOT. Columns: employee_ref (the key the rest of the platform joins on, e.g. hub.fleet.current_driver_ref), full_name, job_title, sub_business, reports_to, work_email, work_mobile, employment_status, worker_type, trainer_id (non-null = is a trainer at all).", 'people find "<name>" FIRST -- it asks all four people-stores and names which answered. To query the table directly: VAULT=/tmp/pbs python3 /tmp/pbs/lg-sql.py --ref rsczwfstwkthaybxhszy "SELECT ... FROM hub.staff_directory". NOT cc-sql -- cc-sql.py is hard-wired to the Command Centre project and silently DROPS a --ref flag, so it answers from the wrong database.', "PAYROLL IS A DIFFERENT DOMAIN -- it has its own rows ('Payroll -- UK (Sygma)' = CC schema payroll, OWNER-PRIVATE; 'Payroll -- Spain'). It was bundled into this row until 5 Aug 2026, which muddled an operational roster with owner-private salary data; do not re-merge them. ⚠ NAME COLLISION, the reason this row must name the ref: public.staff_directory exists in BOTH databases -- the CC one is a contact-card MIRROR ('NOT A ROSTER'), the platform one is the SSOT. An unqualified `staff_directory` query against the CC returns populated, plausible, WRONG rows with no error. mirror-read-guard.py refuses that read; this row is what tells you where to go instead."),
    ...
    ("Gmail label filing homes (label -> Drive folder)", "Cross", "CC Supabase public.gmail_label_homes (one hand-written row per label: label, drive, path, is_bucket)", "cc-sql · written/read by triage-log.py (filing enrichment refuses to guess a home without a row)", "Maps each filing Gmail label to the Drive folder its attachments/extracts belong in. Curated, never derived; add a row when a new filing label is created."),
    ...
    ("Sporting events entered (races: swims/tris etc.)", "Personal", "command-centre repo: lib/sporting-events.ts (hand-authored, renders /m/health/events-entered + English rules sheets in public/sporting-events/)", "Edit the TS array in the repo + push (page sorts by date); page at commandcentre.info/m/health/events-entered", "The events-entered SSOT. NOTE: /m/health sub-pages are NOT public.modules rows, so whereis/map can't see them — this row is the pointer. Race-entry emails (Sportmaniacs/Cronoline) must be processed here + GCal (La Tiñosa miss, 28 Jul 2026)."),
    ...
    ("CD Leak app data", "Canary Detect", "CD-Leak Supabase + Drive: Canary Detect/App Data", "the CRM app (hard-excluded from automated ops)", "Live customer CRM — verify ref + hard-exclude before any Drive op. Per-device operating narrative + per-device SOP live in public.device_notes, surfaced as the System Notes tab on each device page (30 Jul 2026)."),
    ("Sygma Platform", "Sygma", "SygmaSol/sygma-platform (Vercel app, sygmaportal.com) · Supabase rsczwfstwkthaybxhszy (hub schema) · Drive: Sygma Hub/App Data", "the Sygma Platform app (CC reads/surfaces, never owns)", "The one Sygma training app — the merged 'Sygma Portal' + 'Internal Hub' (both app names retired 29 Jul 2026). 'Hub' now means the Google Drive ONLY. CC monitors, never absorbs (#2)."),
    ...
    # 31 Jul 2026: clancy_damages was RETIRED. Sygma kept a parallel damage register alongside
    # Depotnet; its records were merged onto clancy_dn_incidents (the sygma_* fields) and the table
    # dropped. The six that had no Depotnet counterpart went to clancy_unmapped_damages.
    # A table only counts as HOMED when it is named QUALIFIED (public.x) — cc-locator-audit's
    # _homed_table requires it, because bare-word matching used to report unrelated prose as a
    # home. So write every table as public.<name> here, or the locator reports it unhomed even
    # though this row plainly describes it (6 Clancy tables did exactly that, filed 4 Aug 2026).
    ("Clancy damage reviews", "Sygma", "CC Supabase public.clancy_reports + public.clancy_actions + public.clancy_training_courses + public.damage_review_rules + public.clancy_report_facts (verified per-FY facts the board report quotes) + public.clancy_report_lesson_buckets (per-incident three-bucket lesson review behind the board report) + public.clancy_analysis_editions (which edition/basis each published analysis was built from)", "cc-sql · the damage-review engine · /m/clancy-damage-reports · clancy-dn-board-report.py (/m/clancy-damage-year)", "The reports Sygma has written for Clancy (sectioned), the partnership action board, the rollout schedule, and the wording rules the review engine lints against. The three report_* / analysis tables (added 2 Aug 2026) are the board-report working set: quoted facts, the lesson bucket review, and the edition stamp on each published analysis. The DAMAGES themselves are NOT here — they live in the Depotnet register row below. RE-AUTHORED 19 Jul 2026: a hand-edit naming all five was reverted by this cron on 19 Jul 04:01 because it was never written here — every table must be named in the SCRIPT, not the table."),
    ("Clancy service damages (Depotnet register)", "Sygma", "CC Supabase public.clancy_dn_incidents (the register) + clancy_dn_actions + clancy_dn_answers + clancy_dn_files + clancy_dn_index (view) + clancy_dn_rollup (view) + public.clancy_glossary (the section's terms, SSOT for wording - added 2 Aug 2026) + public.clancy_dn_lesson_quality (per-incident lesson-text quality scan: length/duplication/truncation tiers, added 2 Aug 2026) + clancy_unmapped_damages + clancy_damages_snapshot_20260731 + public.clancy_dn_files_snapshot_20260801 (pre-board-report snapshot of clancy_dn_files, audit trail)", "cc-sql · clancy-dn-capture.py / clancy-dn-import.py / clancy-dn-pdf.py · clancy-dn-hub.py + clancy-dn-pages.py + clancy-dn-reports.py + clancy-dn-unmapped.py + clancy-dn-glossary.py (/m/clancy-damage-glossary) · /m/clancy-depotnet", "Every service damage The Clancy Group has logged on Depotnet, group-wide, four financial years. clancy_dn_incidents is the register and also carries Sygma's own review of a damage in its sygma_* fields (merged from the retired clancy_damages, 31 Jul 2026). clancy_unmapped_damages holds the six Sygma records with NO Depotnet counterpart — deliberately outside every count and outside what Genny reads. clancy_damages_snapshot_20260731 is the pre-merge snapshot, kept as the audit trail. Surface = Genny's Damage Depot at /m/clancy-depotnet."),
    ("Clancy Depotnet Genny & CAT data (inspections / actions / coverage / findings)", "Sygma", "CC Supabase public.clancy_dn_gc_inspections + clancy_dn_gc_actions + clancy_dn_gc_coverage (weekly Depotnet exports, idempotent) + clancy_dn_gc_findings (curated: sampling flags, kit serials, evidence assets, platform comparisons - never touched by imports). ALSO public.clancy_dn_inspections - an EARLY PARTIAL export (83 rows, one inspector, one contract) entirely contained in clancy_dn_gc_inspections; superseded, do not read it.", "cc-sql · clancy-dn-gc-import.py (weekly refresh: --inspections --actions --coverage; embeds via voyage, same space as clancy_dn_incidents/actions so damages + reviews search as one system) · clancy-dn-gc-pages.py publishes /m/clancy-genny-cat-reviews", "How Genny & CAT inspections are actually being carried out: every completed usage inspection, the actions raised from them, the per-operative coverage list, and the findings from reading the inspection documents. Reading clancy_dn_inspections instead of the gc_ table made the hub report 83 usage inspections where the section itself reported 5,403 (caught 31 Jul 2026)."),
    ("Bank reconciliation queries (the Xero Discuss replacement)", "Sygma", "Google Sheet 1dIetYBozhkBlr0PRjAdrIiShlC_LdUIuH4lpTnHjjZo -- one tab per bank account. Source exports, statements and receipts in Drive Sygma Private / Accounts / Bank Reconciliation / YYYY-MM-DD (one folder per run, private).", "recon-sheet.py build <Xero recon .xlsx exports> [Capital on Tap .csv] --sheet 1dIetYBozhkBlr0PRjAdrIiShlC_LdUIuH4lpTnHjjZo -- ALWAYS pass --sheet or you create a new spreadsheet and lose Andy's access and notes. Process note [[bank-reconciliation-queries]].", "Xero exposes NO API for unreconciled bank statement lines or the reconciliation Discuss tab, permanently and by policy, so the conversation with the bookkeeper (Andy Jones, editor access) lives in a sheet we own. Each line is identified, categorised against Sygma's own chart, split net/VAT and checked for a matching open bill. Re-running marks reconciled lines DONE and KEEPS them as history rather than deleting. Built 4 Aug 2026."),
    ...
    ("Robin Hood script (Freya's pantomime, editable versions)", "Personal", "Finchy Fairytales Supabase (fnkbiethyuoufgetpozh): public.rhs_doc (the live V3/V4 documents) + public.rhs_doc_rev (a full snapshot per revision, the recovery source) + public.rhs_edit (who changed what, surfaced at /changes)", "the robin-hood-script app (robin-hood-script.vercel.app) · lg-sql.py --ref fnkbiethyuoufgetpozh for reads", "V1/V2 stay frozen in the repo and are NOT here. Deliberately not the CC database: that key bypasses RLS across secrets/banking/payroll. RLS on, no policies, service_role only; writes go through the rhs_apply_edit function."),
    ...
    ("Claude-to-Claude exchange (Pete ⇄ Jim)", "Cross", "Google Chat space 'Claude Exchange' (spaces/AAQA66Jdho0) + Sygma Hub _System/claude-exchange/ (the file half)", "chat-api.py messages spaces/AAQA66Jdho0 15 · chat-api.py attachments / download-attachment for files sent INTO the space · drive-api.py on the Hub folder · protocol = the space's pinned first message + Hub _System/README.md", "The standing channel between Pete's Claude and Jim's Claude (built 4 Aug 2026). Short coordination in the space, long payloads as files in the folder, and the RULE: a file drop with no space message = NOT delivered (request 07 sat unseen). Every message opens [Pete's Claude]/[Jim's Claude]; wired into brain Resume 6b + Compress 7d + closeout I9. No secrets, pointers only. OPERATING MANUAL: vault_notes [[claude-exchange-channel]] (5 Aug 2026) — how to read an attachment, the 4,096-char message limit (split or use the file half), and the trap that ONE Google user ID carries BOTH the human and their Claude, so the [Jim's Claude] tag is the only thing telling them apart."),
...
BACKING = {
    ...
    "Clancy damage reviews": "table:public.clancy_reports",
    "Clancy service damages (Depotnet register)": "table:public.clancy_dn_incidents",
    "Clancy Depotnet Genny & CAT data (inspections / actions / coverage / findings)": "table:public.clancy_dn_gc_inspections",
    "Finance ledger (CC)": "table:public.finance_ledger",
    "Ads (advertising)": "table:public.ads",
    "Family ID / admin": "table:public.family_id",
    "Robin Hood script (Freya's pantomime, editable versions)": "external:finchy-fairytales.rhs",
    ...
    "Staff (Sygma)": "external:rsczwfstwkthaybxhszy/hub.staff_directory",
