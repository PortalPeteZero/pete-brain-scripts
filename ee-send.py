...
def main():
    ...
    # Ask the renderer whether this is its own output (explicit marker), rather than guessing from a
    # colour that any restyle could drop. Falls back to the pre-Aug-2026 colour sniff if the import
    # fails, so a broken import degrades to the old behaviour instead of reporting formatted=False.
    ...
    try:
        formatted = h.looks_formatted(html)
    except Exception:
