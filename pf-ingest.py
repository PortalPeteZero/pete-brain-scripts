...
def cmd_status(a):
    r = subprocess.run(f'cd "{VAULT}" && python3 cc-sql.py "SELECT type, count(*) AS n, count(*) FILTER (WHERE embedding IS NOT NULL AND embedded_hash = content_hash) AS embedded FROM vault_notes WHERE tags && ARRAY[\'passionfit-concepts\'] GROUP BY type ORDER BY n DESC"',
